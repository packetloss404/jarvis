...
// =================================================================
// SIGNED room_hello (member-id slot DoS fix)
// =================================================================
//
// The relay REQUIRES every Room client to sign its room_hello with its ECDSA
// identity. This builder reproduces the EXACT canonical bytes / payload the
// relay's room_auth verifier (and the Rust + desktop-JS clients) expect, so all
// four clients emit a byte-identical signed hello.
//
//   canonical = "jarvis-room-hello-v1" 0x1F session_id 0x1F member_id 0x1F
//               pubkey 0x1F nonce(decimal)
//   payload   = base64(canonical)            // the string actually signed
//   sig       = ECDSA-P256(payload)          // base64 P1363 (Identity.sign)
var RoomHelloSig = {
  DOMAIN: 'jarvis-room-hello-v1',
  SEP: '\x1F',

  payload: function(sessionId, memberId, pubkey, nonce) {
    var canonical = this.DOMAIN + this.SEP + sessionId + this.SEP + memberId +
      this.SEP + pubkey + this.SEP + String(nonce);
    return btoa(canonical);
  },

  build: async function(sessionId, memberId) {
    var pubkey = Identity.pubkeyBase64 || '';
    var nonce = Date.now(); // unix-epoch MILLISECONDS
    var sig = await Identity.sign(this.payload(sessionId, memberId, pubkey, nonce));
...
RoomConnection.prototype._open = function() {
  ...
  ws.onopen = function() {
    ...
    // SIGNED room_hello: the relay rejects an unsigned/forged hello. Signing is
    // async (Web Crypto), so build the hello then send if the socket is still
    // current.
    RoomHelloSig.build(self.channelId, self.memberId).then(function(hello) {
      if (self.ws !== ws || ws.readyState !== WebSocket.OPEN) return;
      ws.send(hello);
      self._startPing();
      if (self.handlers.onOpen) self.handlers.onOpen();
    }).catch(function(e) {
      try { console.error('room_hello signing failed', e); } catch (_) {}
